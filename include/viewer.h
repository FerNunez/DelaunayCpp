...
public:
  ...
  void show(const std::vector<Edge *> &vector_e,
            const std::vector<float2> &vertex) {
    clear();
...
private:
  ...
  void drawVertex(const std::vector<float2> &, int, int, int, int);
...
void Viewer::drawVertex(const std::vector<float2> &vertex, int r, int g, int b,
                        int scale) {
